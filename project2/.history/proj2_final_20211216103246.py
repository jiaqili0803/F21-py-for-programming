...
def printData(music_array):
    print('%-40s\t%-20s\t%-20s' % ('Artist', 'Album', 'Track'))
    for x in music_array:
        print('%-40s\t%-20s\t%-20s' %(x[0], str(x[1]), str(x[2])))
...
def altSortTracks(music_array):
    for x in range(len(music_array)):
        ids = x
        for y in range(x+1, len(music_array)):
            if music_array[ids][2] < music_array[y][2]:
                ids = y
            music_array[x] , music_array[ids] = music_array[ids] , music_array[x]
...
def main():
    ...
    # print
    print('Time Binary Search: ' + str(round(time1, 5)))
    print('Time Sequential Search: ' + str(round(time2, 5)))
    print('Time bubble Sort: ' + str(round(time3, 5)))
    print('Time Selection Sort: ' + str(round(time4, 5)))
    print('Time In Sort: ' + str(round(time5, 5)))
    print('Time Alt Sort: ' + str(round(time6, 5)))
    
    print('#################### Bubble Sort Result: ####################')
    printData(bubbleSortAlbums(music_array))
    print('#################### Insertion Sort Result: ####################')
    printData(inSortTracks(music_array))
...
# A: We can not say so. This time we only have a .cvs.file with 296 lines, data is not enough, We need more data/experiment/comparation.

########## 
# Q: Can you say that one sort is more efficient than another? Why or why not? 

# A: We can not say so. We need more data/experiment/comparation. Same reason as the first question.
